...
from timemoseg.utils.tools import gen_dx_bx
"""
add moving car +moving pedestrain
"""
...
# make边框
def make_contour(img, colour=[255,255,255], double_line=False): # 255 是白色，000是黑色
    h, w = img.shape[:2]
    out = img.copy()
    # Vertical lines
...
def visualise_output(labels, output, cfg):
    ...
    sequence_length = labels['moving_seg'].shape[1] # 7
    b = 0 # 只画 batch里面第一个 
    video = []
    for t in range(sequence_length):
        out_t = []
        # Ground truth
        if cfg.SEMANTIC_SEG.VEHICLE.ENABLED:
            semantic_seg = labels['segmentation'].squeeze(2).cpu().numpy()
            semantic_plot = semantic_colours[semantic_seg[b, t][::-1, ::-1]] #(200, 200, 3)
            semantic_plot = make_contour(semantic_plot)

        
        if cfg.MOVING_SEG.ENABLED:
            movingseg_seg = labels['moving_seg'].squeeze(2).cpu().numpy()
            movingseg_plot = semantic_colours[movingseg_seg[b, t][::-1, ::-1]]
            movingseg_plot = make_contour(movingseg_plot)
        
        # if cfg.INSTANCE_SEG.ENABLED:
        #     unique_ids = torch.unique(labels['instance'][b, t]).cpu().numpy()[1:]  # 不要背景
        #     instance_map = dict(zip(unique_ids, unique_ids))
        #     instance_plot = plot_instance_map(labels['instance'][b, t].cpu(), instance_map)[::-1, ::-1]  # 人+car
        #     instance_plot = make_contour(instance_plot)

        #     center_plot = heatmap_image(labels['centerness'][b, t, 0].cpu().numpy())[::-1, ::-1] # 实例中心
        ...
        # 添加 gt的图-- 添加movingseg_plot gt
        # out_t.append(np.concatenate([instance_plot, future_flow_plot, semantic_plot, center_plot, offset_plot, pedestrian_plot, planning_plot], axis=0))
        # out_t.append(np.concatenate([ semantic_plot,  movingseg_plot], axis=0))
        if cfg.SEMANTIC_SEG.VEHICLE.ENABLED:
            out_t.append(np.concatenate([    semantic_plot,  movingseg_plot  ], axis=0))
        else:
            out_t.append(np.concatenate([  movingseg_plot  ], axis=0))
        ...
        # plot prediction moving_seg msy 
        if cfg.MOVING_SEG.ENABLED:
            movingseg_seg = output['moving_seg'].argmax(dim=2).detach().cpu().numpy()
            movingseg_plot = semantic_colours[movingseg_seg[b, t][::-1, ::-1]]
            movingseg_plot = make_contour(movingseg_plot)
        # else:
        #     movingseg_plot = np.zeros_like(semantic_plot)


        """human moving"""
        # if cfg.MOVING_HUMAN.ENABLED:
        #     movingseg_human = output['moving_human'].argmax(dim=2).detach().cpu().numpy()
        #     movinghuman_plot = semantic_colours[movingseg_human[b, t][::-1, ::-1]]
        #     movinghuman_plot = make_contour(movinghuman_plot)
        # else:
        #     movinghuman_plot = np.zeros_like(semantic_plot)
        ...
        # 添加 预测的所有labels的图 [(1400, 200, 3),(1400, 200, 3)]
        # out_t.append(np.concatenate([instance_plot, future_flow_plot, semantic_plot, center_plot, offset_plot, pedestrian_plot, planning_plot], axis=0))
        if cfg.SEMANTIC_SEG.VEHICLE.ENABLED:
            out_t.append(np.concatenate([    semantic_plot,  movingseg_plot  ], axis=0))
        else:
        ...
        out_t = np.concatenate(out_t, axis=1) # (1400, 400, 3)
        # Shape (C, H, W)
        out_t = out_t.transpose((2, 0, 1))  # (3 1400 400)

        video.append(out_t)

    # Shape (B, T, C, H, W)
    video = np.stack(video)[None] # (1, 7, 3, 1400, 400)  绘制7帧的
    return video
